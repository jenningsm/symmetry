...
PVector[] generatePoints(float breadth, float mHeight, int[] nums){
 PVector[] emptyArray = {};
 return generatePoints(breadth, mHeight, nums, nums.length, emptyArray); 
}

PVector[] generatePoints(float breadth, float mHeight, int[] nums, int depth, PVector[] attractors){
  ...
  return generatePoints(breadth, mHeight, nums, depth - 1, points);
}
